...
def aggResult(t_grid_data, result):
	# aggregate the result
	print("enter aggResult")
	print("grid_lat*grid_lon:")
	print(grid_lat*grid_lon)
	for z in np.arange(grid_lat*grid_lon):
		# For all variables
		key_idx = 0
		print("varnames in aggResult")
		print(varnames)
		print("sts_name")
		print(sts_name)
		for key in varnames:
...
# This is the main program for using concurrent to speed up the whole process
	...
	grid_data = sc.parallelize(list(file_pairs), file_num).map(lambda x: run_modis_aggre(x[0],x[1],day_in_year,shift_hour,NTA_lats,NTA_lons,grid_lon,grid_lat,gap_x,gap_y,filenum,sts_switch,varnames,intervals_1d,intervals_2d,var_idx, spl_num, sts_name, histnames)).reduce(aggResult)
	...
	subname = 'serial_output_31days.h5'
