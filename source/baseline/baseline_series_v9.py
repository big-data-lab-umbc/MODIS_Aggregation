...
V9 Updates: Added scale_factor, add_offset and _Fillvalue as attributes to each variabes.
			Found the problem in reading temperature and fixed it by setting the value manually.
"""
...
def read_MODIS(varnames,fname1,fname2): 
	...
	for key in varnames:
		...
		elif key == 'cloud_top_temperature_1km':
	...
	return lat,lon,data,longname_list,fillvalue_list,scale_list,offst_list
...
def run_modis_aggre(fname1,fname2,NTA_lats,NTA_lons,grid_lon,grid_lat,gap_x,gap_y,hdfs, \
	...
	hdfs = np.array(hdfs)
	for j in range(2):#hdfs:
		print("File Number: {} / {}".format(j,hdfs[-1]))
	
		# Read Level-2 MODIS data
		lat,lon,data,longname_list,fillvalue_list,scale_list,offst_list = read_MODIS(varnames,fname1[j],fname2[j])
		CM = data['CM']
	...
	return grid_data,longname_list,fillvalue_list,scale_list,offst_list
...
def addGridEntry(f,name,units,long_name,fillvalue,scale_factor,add_offset,data):
	...
	'''
	original_data = (data-add_offset)/scale_factor
...
# This is the main program for using concurrent to speed up the whole process
	...
	grid_data,longname_list,fillvalue_list,scale_list,offst_list = \
	run_modis_aggre(fname1,fname2,NTA_lats,NTA_lons,grid_lon,grid_lat,gap_x,gap_y,filenum, \
					grid_data,sts_switch,varnames,intervals_1d,intervals_2d,var_idx)
	...
	ff=h5py.File(l3name+'_baseline_example_v8.h5','w')
	...
	print(l3name+'_baseline_example_v8.h5 Saved!')
