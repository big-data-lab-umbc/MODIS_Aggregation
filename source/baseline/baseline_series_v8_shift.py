...
def read_filelist(loc_dir,prefix,yr,day,time,shift_hr,fileformat):
	filename = []
	...
	for i in range(len(time)):
		...
		str = os.popen("ls "+ loc_dir + prefix + yc + dc + "."+ tc +"*."+fileformat).read()
...
# This is the main program for using concurrent to speed up the whole process
	...
	for dt in rrule(DAILY, interval=1, dtstart=start, until=until):
		...
		# Adjust to 3 hours previous/after for the orbit gap/overlap problem
		shift_hour = 3
		#time    = np.roll(np.arange(24),shift_hour*-1)
		...
		print(year,day_in_year,time)
		
		# Start reading Level-2 files 
		fname_tmp1 = read_filelist(MYD06_dir,MYD06_prefix,year,day_in_year,time,shift_hour,fileformat)
		fname_tmp2 = read_filelist(MYD03_dir,MYD03_prefix,year,day_in_year,time,shift_hour,fileformat)
