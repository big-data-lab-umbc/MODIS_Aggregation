...
# This is the main program for using concurrent to speed up the whole process
	...
	#print("Total number of files : {}".format(len(fname1)))
	...
	results = np.asarray(run_modis_aggre(fname1,fname2,TOT_pix,CLD_pix,hdfs))
	...
	if rank == 0:
		...
		np.savetxt("cld_fraction_mpi_new.dat", ave_fra_grid, fmt="%10.4f")
